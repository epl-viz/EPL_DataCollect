...
class Test_PythonException(Plugin.Plugin, unittest.TestCast):       # name your class plugin like your python module, and let it derive from Plugin.Plugin !
  def getID(self):
  ...
  def run(self):                          # (optional, but without it well...) run method
    ...
    if self.getCycle().getCycleNum() > 1:
      self.fail("[python] exceptions should stop the specific plugin !")
